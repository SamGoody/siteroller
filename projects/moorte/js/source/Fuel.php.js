/**
 * Script:
 *   Fuel.php.js - PHP language file for Lighter.js
 *
 * License:
 *   MIT-style license.
 * 
 ...
 *
 * Known Issues:
 *  - Missing PHP specific regex patterns to match regular expressions.
 * 
 */
Fuel.php = new Class({
	...
	initialize: function(lighter, flame, options, wicks) {
		...
		this.delimiters = new Hash({
			start: this.strictRegex('<?php', '<?=', '<%', '<script language="php">'),
			end:   this.strictRegex('?>', '%>', '</script>')
		});
		
		this.parent(lighter, flame, options, wicks);
	}
});